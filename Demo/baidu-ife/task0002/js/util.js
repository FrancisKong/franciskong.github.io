...
function simpleTrim(str) {
    ...
    for(var i=0;i<str.length-1;i++){ if(str.charat(i)="="　"||str.charAt(i)=="\t"||str.charAt(i)=="" "){="" startindex++;="" }="" else{="" break;="" for="" (var="" i="str.length" -="" 1;="">= 0; i--) {
...
function each(arr, fn) {
    for(var i=0;i<arr.length;i++){ fn(arr[i],i);="" }="" 获取一个对象里面第一层元素的数量，返回一个整数="" function="" getobjectlength(obj)="" {="" var="" i="0;" for(var="" a="" in="" object.keys(obj)){="" i++;="" return="" i;="" 为element增加一个样式名为newclassname的新样式="" addclass(element,="" newclassname)="" element.classname+="newClassName;" 移除element中的样式oldclassname="" removeclass(element,="" oldclassname)="" classname="element.className;" if(classname!="null&&element.className.indexOf(oldClassName)">-1){
